...
public class CardService {
    ...
    public void createCard(CardDto cardDto) {
        Optional<BankAccountEntity> bankAccountEntity = bankAccountRepository.findById(cardDto.getBankAccountDto().getId());
        if (bankAccountEntity.isPresent()) {
            CardEntity newCard = CardMapper.toCardEntity(cardDto);
            newCard.setBankAccountEntity(bankAccountEntity.get());
